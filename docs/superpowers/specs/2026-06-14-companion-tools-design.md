...
- [x] 更新章首「本章路由」表，增补「进阶」行
- [x] 「选型与组合」段末加交叉引用至本机进阶节
- [x] 新增「本机进阶工具一览」：五张场景表 + 黄金搭档 + 优先上手
- [x] 精简「外延」节：删除三行本机候选表，节末加回链句
- [x] 自检清单可增补一题：能说出 direnv + worktree 的配合
- [x] `pnpm build` 通过
